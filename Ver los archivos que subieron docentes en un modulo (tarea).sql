...
f.filename as nombreDelArchivo,
f.source as fuente,
f.author as autor,
a.name as nombreActividad,
a.intro as descripcion,
a.activity as instruccionesActividad
...
inner join prefix_assign as a on a.id = cm.instance and a.course = course.id
WHERE 
	course.id in (15920)and (/*(r.shortname = 'student') or */(r.shortname = 'editingteacher')) and ue.status = 0
and	f.filesize > 0
